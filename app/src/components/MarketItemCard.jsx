...
function MarketItemCard_(props, ref) {
  ...
  const { itemId, onAddShield, onSell, price, marketId, setLoading, ...rest } =
    props;
  ...
  React.useEffect(() => {
    if (itemId) {
      const shieldValue =
        nftContract.methods["tokenShieldValue"].cacheCall(itemId);
      const metadata = nftContract.methods["tokenURI"](itemId)
        .call()
        .then((uri) => {
          if (uri) {
  ...
  const shieldValue = (
    drizzleContext.drizzleState.contracts.ShieldNFT.tokenShieldValue[
      dataKeys.shieldValue
    ]?.value / 100
  ).toFixed(2);
  ...
  return React.useMemo(() => {
    return (
      <PlasmicMarketItemCard
        ...
        removeButton={{
          ...(!isProfileItem && {
            children: isAllowed ? `Buy now: ${price / 100} SHIELD` : `Approve`,
          }),
          onClick: async (event) => {
            ...
            if (isProfileItem) {
              ...
              try {
                if (isAllowed) {
                  await marketplaceContract.methods["buyNFT"](marketId).send();
                } else {
                  await shieldContract.methods["approve"](
                    marketplaceContract.address,
                    maxAllowance
                  ).send();
                }
