...
function NftSellWidget_(props, ref) {
  ...
  React.useEffect(() => {
    if (itemId) {
      ...
      const approvedKey = nftContract.methods["isApprovedForAll"].cacheCall(
        account,
  ...
  const isApproved =
    drizzleContext.drizzleState.contracts.ShieldNFT.isApprovedForAll[
      dataKeys.approved
    ]?.value;
  return (
    <PlasmicNftSellWidget
      ...
      sellButton={{
        isDisabled:
          (amount == 0 || amount * 100 > shieldValue * 100) && isApproved,
        onClick: async () => {
          setLoading(true);
          try {
            ...
            if (isApproved) {
              onSubmit();
              await marketplaceContract.methods["addNFT"](
                itemId,
                amount * 100
              ).send();
