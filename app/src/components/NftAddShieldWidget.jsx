...
function NftAddShieldWidget_(props, ref) {
  ...
  React.useEffect(() => {
    const allowanceKey = forgeContract.methods["allowance"].cacheCall(
      account,
      nftContract.address
    );
  ...
  const allowanceAmount =
    drizzleContext.drizzleState.contracts.ForgeToken.allowance[
      allowanceKeys.forge
    ]?.value;
  const isAllowed = allowanceAmount != 0;

  return (
    <PlasmicNftAddShieldWidget
      variants={{
        approve: !isAllowed,
      }}
      ...
      mintButton={{
        ...
        onClick: async () => {
          ...
          try {
            if (!isAllowed) {
              await forgeContract.methods["approve"](
                nftContract.address,
                maxAllowance
              ).send();
              ...
              onSubmit();
              await drizzle.contracts.ShieldNFT.methods["addTokenShieldValue"](
                itemId,
                amount * 100
              ).send();
            }
